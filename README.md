...
3. BeginNotify
 + 콤보 공격 시 콤보 카운트를 증가하여 다음 공격 모션이 나가야 함을 DoAction에 전달하도록 하였습니다.
...
6. EquipNotify
 + 캐릭터가 무기를 장착했음을 알려주는 노티파이 입니다.

7. ComboNotify(State)
 + 공격 모션 중 콤보 노티파이 스테이트 구간 안에서 다시 공격할 경우 다음 콤보 공격 모션이 재생되도록 하였습니다.

7. CollisionNotify(State)
 + 애니메이션에 맞게 공격 모션 지정 시 충돌체를 지정하는 노티파이 스테이트 입니다.
 + Collision의 노티파이 스테이트 구간 안에 충돌체가 있을 경우 데미지를 받습니다.
